function _1(md){return(
  ...
  function _chart(d3,width,height,xAxis,data,x,y,r,colour,invalidation,networks,margin,yAxis)
  {
      ...
      .join("circle")
        ...
        .call(d3.drag()
          ...
          .on("end", d => {
            ...
            d3.selectAll("circle")
              .style("fill", function(d, i){ return colour(d.ID); })
  ...
  const _height = () => {return(
  400
  )}
  
  const _margin = () => {return(
  {top: 50, right: 100, bottom: 50, left: 200}
  )}
  
  const _data = (FileAttachment) => {return(
  FileAttachment("data.json").json().then(data => data.map((d, i) => ({id: i + 1, ...d})))
  )}
  
  const _networks = (data) => {return(
  [...new Set(data.map(d => d.network))]
  )}
  
  const _x = (d3,data,margin,width) =>{return(
  d3.scaleLinear()
  ...
  const _y = (d3,networks,height) => {return(
  d3.scaleBand()
    .domain(networks)
    .range([height / 2, height / 2])
  )}
  
  const _xAxis = (margin,d3,x,width) => {return(
  g =>
    g
  ...
  const _r = (d3,data) => {return(
  d3.scaleSqrt()
    .domain(d3.extent(data, d => d.volume24h))
    .range([7, 20])
  )}
  
  const _colour = (d3,data) => {return(
  d3.scaleSequential(d3.extent(data, d => d.volume24hCap), d3.interpolatePlasma)
  )}
  
  const _d3 = (require) => {return(
  require("d3@5")
  )}
  
  const _html =  async (require) => {return(
